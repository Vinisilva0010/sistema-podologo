...
export default function MyBookingsPage() {
  ...
  // Estado para armazenar os agendamentos encontrados (mock temporário)
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      setMyAppointments([
        {
          id: "123",
          serviceName: "Unha Encravada",
          date: "2026-03-05", 
          startTime: "10:00",
          status: "confirmed",
          notes: "Paciente relatou dor forte no dedão direito."
        }
      ]);
      setHasSearched(true);
      setLoading(false);
  ...
  const handleCancel = (id: string) => {
    // Alerta nativo para evitar clique acidental (UX de segurança)
    const isSure = window.confirm("Tem certeza que deseja cancelar esta consulta? Essa ação não pode ser desfeita.");
    
    if (isSure) {
      // Atualiza o estado visualmente mudando o status para "cancelled"
      setMyAppointments(prev => 
        prev.map(appt => appt.id === id ? { ...appt, status: "cancelled" } : appt)
      );
      
      // TODO: Aqui vamos chamar a Server Action no futuro para atualizar no Firebase
              ...
              <div className="flex flex-col gap-6">
                ...
                {myAppointments.map((appt) => (
                  <div key={appt.id} className="bg-white border-4 border-black shadow-[6px_6px_0px_0px_#000] overflow-hidden group transition-all duration-300">
                    ...
                    {/* Cabeçalho do Card (Muda de cor se cancelado) */}
                    ...
                    <div className={`p-6 ${appt.status === "cancelled" ? "opacity-60 grayscale" : ""}`}>
                      ...
                      {/* Ações (Só aparecem se NÃO estiver cancelado) */}
                      {appt.status !== "cancelled" && (
                        <div className="mt-8 pt-6 border-t-4 border-black flex flex-col sm:flex-row gap-4">
                          
                          <button 
                            onClick={() => handleCancel(appt.id)}
                            className="flex-1 flex items-center justify-center gap-2 bg-red-500 text-white border-4 border-black p-3 font-black uppercase text-sm shadow-[4px_4px_0px_0px_#000] hover:bg-red-600 hover:-translate-y-1 hover:shadow-[6px_6px_0px_0px_#000] active:translate-y-[4px] active:shadow-none transition-all"
                          >
                          ...
                          <a 
                            ...
                            className="flex-1 flex items-center justify-center bg-yellow-400 text-black border-4 border-black p-3 font-black uppercase text-sm shadow-[4px_4px_0px_0px_#000] hover:bg-yellow-300 hover:-translate-y-1 hover:shadow-[6px_6px_0px_0px_#000] active:translate-y-[4px] active:shadow-none transition-all"
                          >
                            Remarcar no Zap
                          </a>
